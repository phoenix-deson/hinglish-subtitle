...
def editor_parse_srt(content):
    ...
    for block in blocks:
        ...
        try:
            ...
            start = editor_parse_time(start_text)
            end = editor_parse_time(end_text)
        except Exception:
            continue
...
def editor_build_ass(subtitles, settings):
    primary = editor_ass_color(settings["text_color"])
    outline = editor_ass_color(settings["outline_color"])
    ...
    for item in subtitles:
        text = editor_ass_escape(item["text"])
        if settings["highlight"]:
            text = "{\\c" + editor_ass_color(settings["highlight_color"]) + "}" + text
        if settings["bold"]:
            text = "{\\b1}" + text
        if settings["underline"]:
            text = "{\\u1}" + text
        lines.append(
            f"Dialogue: 0,{editor_ass_time(item['start'])},{editor_ass_time(item['end'])},Default,,0,0,0,,{text}"
        )
    return "\n".join(lines) + "\n"
        ...
        if not subtitles:
            ...
            for i, item in enumerate(st.session_state.editor_subtitles, 1):
                with st.expander(f"#{i}  ·  {editor_format_time(item['start'])} → {editor_format_time(item['end'])}", expanded=(i == 1)):
                    a, b = st.columns(2)
                    start_text = a.text_input("Start", editor_format_time(item["start"]), key=f"start_{i}")
                    end_text = b.text_input("End", editor_format_time(item["end"]), key=f"end_{i}")
